...
export default function RegisterPage() {
  ...
  const updateProfile = (field: keyof UserProfile, value: string) => { setProfile((prev) => ({ ...prev, [field]: value })); };
  ...
  const canProceed = () => { switch (step) { case 1: return profile.gender !== ""; case 2: return profile.grade !== ""; case 3: return profile.university !== ""; default: return true; } };

  const steps = [
    { title: "性別を選択してください", icon: User, options: ["男性", "女性", "その他", "回答しない"], field: "gender" as keyof UserProfile },
    { title: "学年を選択してください", icon: GraduationCap, options: ["1年生", "2年生", "3年生", "4年生", "5年生", "6年生", "その他"], field: "grade" as keyof UserProfile },
    { title: "大学名を選択してください", icon: Building2, options: ["浜松医科大学"], field: "university" as keyof UserProfile, note: "Phase 1では浜松医科大学のみ対応しています" },
    { title: "所属している部活・サークルを教えてください（任意）", icon: Dumbbell, options: ["運動部", "文化部", "医療系サークル", "その他", "所属していない"], field: "club" as keyof UserProfile, optional: true },
    { title: "希望診療科を選択してください（任意）", icon: Stethoscope, options: ["内科", "外科", "小児科", "産婦人科", "整形外科", "精神科", "皮膚科", "眼科", "耳鼻咽喉科", "その他", "未定"], field: "specialty" as keyof UserProfile, optional: true },
  ];
    ...
    <div className="min-h-screen bg-gradient-to-b from-pink-50 to-white flex flex-col">
      <div className="w-full max-w-lg mx-auto p-6 flex-1 flex flex-col animate-fade-in">
        ...
        <div className="text-center mb-8">
          ...
          {currentStep.note && <p className="text-xs text-gray-500 mt-2">{currentStep.note}</p>}
          {currentStep.optional && <p className="text-xs text-pink-500 mt-2">※ スキップ可能</p>}
        </div>
